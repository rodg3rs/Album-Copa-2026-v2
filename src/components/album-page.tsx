...
function AlbumContent({ tipo }: { tipo: "A" | "R" }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <section className="shrink-0 border-b-2 border-border bg-background px-3 py-1.5 shadow-sm">
      ...
      {msg && <p className="mx-4 mt-3 shrink-0 rounded-lg bg-primary/10 px-3 py-2 text-sm font-medium text-primary">{msg}</p>}
          ...
          <div className="divide-y divide-border pb-2">
            {groups.map((g, idx) => (
              <div key={`${g.key}-${idx}`} className="flex items-center gap-1 px-1 py-1">
                ...
                <div className="grid min-w-0 flex-1 grid-cols-[repeat(20,minmax(0,1fr))] gap-0.5">
                  {g.stamps.map((s) => {
